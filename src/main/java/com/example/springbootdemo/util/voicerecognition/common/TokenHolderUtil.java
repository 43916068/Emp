package com.example.springbootdemo.util.voicerecognition.common;
...
public class TokenHolderUtil {
     ...
     */
    public TokenHolderUtil(String apiKey, String secretKey, String scope) {
        this.apiKey = apiKey;
     ...
     * @throws DemoExceptionUtil http接口返回不是 200, access_token未获取
     * @throws JSONException 
     */
    public void resfresh() throws IOException, DemoExceptionUtil, JSONException {
     ...
     * @throws DemoExceptionUtil
     * @throws JSONException 
     */
    private void parseJson(String result) throws DemoExceptionUtil, JSONException {
        JSONObject json = new JSONObject(result);
        if (!json.has("access_token")) {
            // 返回没有access_token字段
            throw new DemoExceptionUtil("access_token not obtained, " + result);
        }
        if (!json.has("scope")) {
            // 返回没有scope字段
            throw new DemoExceptionUtil("scopenot obtained, " + result);
        }
        if (!json.getString("scope").contains(scope)) {
            throw new DemoExceptionUtil("scope not exist, " + scope + "," + result);
        }
